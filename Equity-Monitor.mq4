...
int init() {
   ...
   if(Equity_Max_Alert != 0) alert_upper_active = true;
   else alert_upper_active = false;
   if(Equity_Min_Alert != 0) alert_lower_active = true;
   else alert_lower_active = false;
...
void CalculateHistoryBars() {
   ...
   for(int i = OpenBar[0]; i >= DrawEndBar; i--) {
      ...
      if(Only_Trading)  {
         Equity[i] = NormalizeDouble(ResultProfit + ProfitLoss,2);
      } else              {
         Equity[i] = NormalizeDouble(ResultBalance + ProfitLoss,2);
      }
      if(Show_Balance)  {
         Balance[i] = NormalizeDouble(Only_Trading ? ResultProfit : ResultBalance,2);
      }
      if(Show_Margin)   {
         Margin[i] = NormalizeDouble(SumMargin,2);
      }
      if(Show_Free)     {
         Free[i] = NormalizeDouble(ResultBalance + ProfitLoss - SumMargin,2);
      }
      if(Show_Zero)     {
         Zero[i] = 0;
      }
...
void CalculateLastBar() {
   ...
   if(!using_filters) {
      ...
      if(Only_Trading)  {
         Equity[0] = ResultProfit + ProfitLoss;
      } else              {
         Equity[0] = AccountEquity();
      }
      if(Show_Balance)  {
         Balance[0] = Only_Trading ? ResultProfit : ResultBalance;
      }
      if(Show_Margin)   {
      ...
      if(Only_Trading)  {
         Equity[0] = NormalizeDouble(ResultProfit + ProfitLoss,2);
      } else              {
         Equity[0] = NormalizeDouble(ResultBalance + ProfitLoss,2);
      }
      if(Show_Balance)  {
         Balance[0] = NormalizeDouble(Only_Trading ? ResultProfit : ResultBalance,2);
      }
      if(Show_Margin)   {
         Margin[0] = NormalizeDouble(SumMargin,2);
      }
      if(Show_Free)     {
         Free[0] = NormalizeDouble(ResultBalance + ProfitLoss - SumMargin,2);
      }
      if(Show_Zero)     {
         Zero[0] = 0;
      }
...
void ReadOrder(int n) {
   ...
   if(swapdays > 0) DaySwap[n] = Swap[n] / swapdays;
   else DaySwap[n] = 0.0;
...
int PrepareFile() {
   ...
   if(handle < 0) {
      Alert("Error #",GetLastError()," while opening data file.");
      return(handle);
   }
